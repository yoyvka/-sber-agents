...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = None) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        llm_model: модель для синтеза (по умолчанию из config.MODEL)
    ...
    # Используем модель из config, если не указана
    model = llm_model or config.MODEL or "gpt-4o"
    
    # Создаем ChatOpenAI с настройками из config
    # ChatOpenAI автоматически берет api_key и base_url из переменных окружения,
    # но мы можем передать их явно, если они есть в config
    llm_kwargs = {
        "model": model,
        "temperature": 0.7
    }
    # Передаем api_key и base_url только если они установлены в config
    # Иначе ChatOpenAI попытается взять их из переменных окружения
...
def upload_to_langsmith(dataset_path: str, dataset_name: str):
    ...
    # Инициализируем Client с явной передачей API ключа
